...
export default function JoinPage() {
  const [joined, setJoined] = useState<StudentJoinInput | null>(null);
  if (joined) return <main className="join-page"><section className="join-shell">
    ...
    <Card className="join-success"><Tag color="green">Đã vào lượt kiểm tra</Tag>
      ...
      <p>Giảng viên đang chuẩn bị mở ba câu kiểm tra cho mã <strong>{joined.sessionCode}</strong>.</p>
  ...
  return <main className="join-page"><section className="join-shell">
    ...
    <Card className="join-card">
      <Form<StudentJoinInput> layout="vertical" requiredMark={false} initialValues={{ sessionCode: "VL-A2" }}
        onFinish={values => setJoined({ sessionCode: values.sessionCode.trim().toUpperCase(), displayName: values.displayName.trim() })}>
