...
export default function NewLessonPage() {
  ...
  return <AppLayout>
    ...
    <Card className="course-builder-card">
      ...
      <div className="course-builder-grid lesson-setup-grid">
        <div className="lesson-upload-stack"><section className="builder-panel lesson-upload-panel"><p className="eyebrow">BÀI GIẢNG</p><h3>{uploadedFileName || "AI & LLM Foundation"}</h3><Upload.Dragger accept=".ppt,.pptx,.pdf" maxCount={1} beforeUpload={file => chooseDemoUpload(file.name)} onRemove={() => { setUploadedFileName(null); setCatalog(null); }} className="mock-upload"><p><strong>{uploadedFileName || "Kéo thả PPTX hoặc PDF vào đây"}</strong></p><p className="muted">Hỗ trợ định dạng PPT, PPTX và PDF.</p></Upload.Dragger></section><div className="lesson-upload-actions"><Button type="primary" loading={loading} disabled={!uploadedFileName} onClick={() => void analyzeLesson()}>Phân tích bài giảng</Button></div></div>
