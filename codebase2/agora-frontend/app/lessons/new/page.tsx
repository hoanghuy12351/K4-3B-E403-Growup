...
export default function NewLessonPage() {
  ...
  return <AppLayout><div className="page-heading"><div><p className="eyebrow">BÀI GIẢNG MỚI</p><h1>Chọn tài liệu bài giảng</h1>
    <p className="muted">Slide D1 có luồng demo checkpoint sẵn; tài liệu khác tiếp tục dùng luồng tạo checkpoint hiện tại.</p></div></div>
    ...
    <Card title="Chọn nguồn tạo checkpoint" className="recent-lesson-card">
      ...
      {loading ? <Spin tip="Đang tải tài liệu..." /> : <>
        {presetDemo && <Card size="small" style={{ marginBottom: 16, borderColor: selectedSource === "preset" ? "#1677ff" : undefined }}>
          <Radio checked={selectedSource === "preset"} onChange={() => setSelectedSource("preset")}><strong>{presetDemo.lesson.title} — Demo D1</strong><br />
            <span className="muted">Checkpoint đã chuẩn bị sẵn, không gọi AI. Hệ thống chỉ mô phỏng xử lý trong vài giây.</span></Radio>
          {selectedSource === "preset" && <div style={{ marginTop: 16 }}>
            <p><strong>{presetDemo.agentMessage}</strong></p>
        ...
        {materials.length === 0 ? <Alert type="info" showIcon title="Chưa có tài liệu đã tải" description="Bạn vẫn có thể dùng demo D1 ở trên hoặc tải thêm PDF/PPTX." /> :
        ...
        <div style={{ marginTop: 24 }}><Button type="primary" size="large" disabled={!canCreate} loading={submitting} onClick={createSession}>{selectedSource === "preset" ? "Tạo checkpoint demo để duyệt" : "Tạo checkpoint để duyệt"}</Button></div>
      </>}
    </Card>
  </AppLayout>;
}
