...
def classify(payload: ClassificationRequest, teacher: Annotated[Teacher, Depends(current_teacher)]) -> ClassificationResponse:
    """Classify an existing generated question; clients never provide a rubric."""
    try:
        session = service.get_session(payload.session_id)
        service.require_teacher(session.id, teacher.id)
